...
export class CartService implements OnDestroy {
  ...
  constructor(private token: TokenService, private service: ProductApiService) { }
  ...
  changeDateTime(deliveryDate: string, idDeliveryTime: number): void {
    if (this.token.isAuthenticated()) {
      ...
      localStorage.setItem(this._key, btoa(JSON.stringify(val)));
      this.getCartItems();
    }
  }

  private getLocalCartItems(): CartItem[] {
    const values = localStorage.getItem(this._key);
    if (values === undefined ||
        values === null ||
        values === ''
    ) {
  ...
  private addLocalCartItem(item: CartItem): void {
    ...
    localStorage.setItem(this._key, btoa(JSON.stringify(values)));
  }
