...
export class CustomInterceptor implements HttpInterceptor, OnDestroy {
  ...
  private handleResponseError(error: HttpErrorResponse, request?: HttpRequest<any>, next?: HttpHandler): any {
    switch (error.status) {
      case 400: //Business error
        if (error.error?.message) {
          if (error.error?.persist) {
            this.snack.open(error.error.message, this._discart);
          } else {
      ...
      case 429: //Too Many Requests
          ...
          this.snack.open(
            error.error?.message || `La cuenta se ha bloqueado, intenta dentro de ${minutes}`,
            this._discart,
            { duration: this._time }
          );
          break;
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    const base = 'https://localhost:44392/';
