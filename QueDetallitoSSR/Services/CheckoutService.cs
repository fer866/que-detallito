...
namespace QueDetallito.Services
{
    ...
    public class CheckoutService : ICheckoutService
    {
        ...
        private async Task<(int, int)> CreateCustomerOrder(OrderPost post)
        {
            ...
            var yearParam = new { year = DateTime.Now.LocalTime().Year };
            var idOrder = await conn.QuerySingleOrDefaultAsync<int>("select isnull(max(id),0) + 1 from Sales.Orders where orderYear = @year", yearParam);
            ...
            {
                ID = idOrder,
                OrderYear = DateTime.Now.LocalTime().Year,
            ...
            //Vacía el carrito
            var param = new { post.IdCustomer };
            await conn.ExecuteAsync("delete Sales.Cart where idCustomer = @IdCustomer", param);
            ...
            if (first == 1)
            {
                discountParam = new DiscountsModel
                {
                    ...
                    Discount = 15,
                    ValidFrom = DateTime.Now.LocalTime(),
                    ValidTo = DateTime.Now.LocalTime().AddMonths(1),
        ...
        private async Task SendOrderEmail(OrderModel order, IEnumerable<CartItem> cartItems, DiscountsModel extraDiscount = null)
        {
            ...
            string body = string.Format(builder.HtmlBody,
                ...
                discount,
                total,
                $"{order.DeliveryDate.ToString("d 'de' MMMM 'de' yyyy")} {orderDetails.DeliveryTime}",
